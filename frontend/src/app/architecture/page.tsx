...
export default function ArchitectureDashboard() {
  ...
  return (
    <div className="h-screen overflow-y-auto bg-background text-foreground font-sans">
      {/* HERO */}
      <div className="relative overflow-hidden border-b border-slate-200 bg-gradient-to-b from-indigo-950 to-white pt-20 pb-14">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-indigo-900/10 rounded-full blur-3xl" />
          <div className="absolute top-10 right-1/4 w-[400px] h-[400px] bg-indigo-800/10 rounded-full blur-3xl" />
        </div>
        <div className="relative z-10 max-w-6xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-900/20 border border-indigo-900/30 text-indigo-400 text-xs font-bold uppercase tracking-widest mb-6">
            <Cpu className="w-3.5 h-3.5" /> Phase 13 · Complete System Reference
          </div>
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-5 bg-clip-text text-transparent bg-gradient-to-br from-indigo-400 to-slate-200">
            Aegis Clinical AI
          </h1>
          <p className="text-lg text-slate-400 max-w-3xl mx-auto leading-relaxed">
            A <strong className="text-indigo-400">13-Phase multi-agent orchestration system</strong> for zero-hallucination clinical decision support. Combines LangGraph agentic workflows, hybrid RAG retrieval, multimodal pipelines, and human-in-the-loop governance.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
            {["LangGraph", "FastAPI", "Next.js 15", "Qdrant", "Neo4j", "Groq / Llama-3", "PubMed Live", "HITL Governance"].map(t => (
              <span key={t} className="px-3 py-1 rounded-full border border-slate-800 bg-slate-950 text-slate-350 text-xs font-semibold">{t}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        {activeTab === "overview" && (
          <div className="space-y-14 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {PHASES.map((p, i) => (
                <div key={i} className={`p-4 rounded-xl border transition-all ${p.highlight ? "border-indigo-900/30 bg-indigo-950/40 shadow-sm" : "border-slate-800 bg-slate-950"}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-slate-900 text-slate-350">Phase {p.phase}</span>
                    {p.highlight && <span className="text-[9px] font-bold text-indigo-400 uppercase">Core</span>}
                  </div>
                  <h3 className={`font-bold text-sm mb-1 ${p.highlight ? "text-indigo-400" : "text-slate-200"}`}>{p.title}</h3>
                  <p className="text-xs text-slate-400 leading-relaxed">{p.desc}</p>
                </div>
              ))}
            </div>

            <div className="p-8 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
              <h3 className="font-bold text-lg text-slate-200 mb-6 flex items-center gap-2"><GitBranch className="text-indigo-400 w-5 h-5" /> High-Level Architecture Flow</h3>
              <div className="grid md:grid-cols-5 gap-2 items-center text-center text-xs font-semibold">
                {[
                  { label: "User Query", bg: "bg-slate-900 text-slate-200 border border-slate-800" },
                  { label: "→", bg: "", text: "text-slate-400" },
                  { label: "Orchestration Planner", bg: "bg-indigo-950 text-indigo-400 border border-indigo-900/20" },
                  { label: "→", bg: "", text: "text-slate-400" },
                  { label: "Parallel Retrieval\n(Qdrant + Neo4j + PubMed)", bg: "bg-emerald-50 text-emerald-800 border border-emerald-200/50" },
                ].map((b, i) => b.bg
                  ? <div key={i} className={`${b.bg} rounded-lg py-3 px-2 whitespace-pre-line`}>{b.label}</div>
                  : <div key={i} className={`${b.text} text-2xl`}>{b.label}</div>
                )}
                <div className="md:col-span-5 flex items-center justify-center gap-2 mt-2 text-slate-400 text-xl font-bold">↓</div>
                {[
                  { label: "Evidence Evaluator\n& Contradiction Check", bg: "bg-amber-50 text-amber-800 border border-amber-200/50" },
                  { label: "→", bg: "", text: "text-slate-400" },
                  { label: "Clinical Reasoning\n& Validation", bg: "bg-violet-50 text-violet-800 border border-violet-200/50" },
                  { label: "→", bg: "", text: "text-slate-400" },
                  { label: "Governance Review\n& Final Response", bg: "bg-rose-50 text-rose-800 border border-rose-200/50" },
                ].map((b, i) => b.bg
                  ? <div key={i} className={`${b.bg} rounded-lg py-3 px-2 whitespace-pre-line`}>{b.label}</div>
                  : <div key={i} className={`${b.text} text-2xl`}>{b.label}</div>
                )}
        ...
        {activeTab === "agents" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {AGENTS.map((a, i) => (
                <div key={i} className="p-5 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm hover:shadow-md/5 transition-all flex flex-col gap-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Agent {i + 1}</span>
                      <h3 className="font-bold text-slate-200 mt-0.5">{a.name}</h3>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-auto pt-2 border-t border-slate-800">
                    {a.tags.map(t => (
                      <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-slate-900 text-slate-350 border border-slate-800 font-semibold">{t}</span>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="p-6 rounded-2xl border border-amber-200 bg-amber-50/50">
              <h3 className="font-bold text-amber-800 mb-3 flex items-center gap-2"><Zap className="w-4 h-4 text-amber-600" /> Adaptive Execution: The Orchestrator&apos;s Rules</h3>
              <div className="grid md:grid-cols-2 gap-3 text-sm text-slate-350">
                {[
        ...
        {activeTab === "rag" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {RAG_ENGINES.map((e, i) => (
                <div key={i} className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm flex flex-col gap-3 hover:border-indigo-500/40 transition-colors">
                  ...
                  <h3 className="font-bold text-slate-200">{e.title}</h3>
                  ...
                  <div className="mt-auto space-y-1.5 pt-3 border-t border-slate-800">
                    ...
                    {e.useCases.map((u, j) => (
                      <div key={j} className="flex items-start gap-1.5 text-xs text-slate-350">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                <h3 className="font-bold text-slate-200 mb-4 flex items-center gap-2"><Layers className="text-indigo-400 w-5 h-5" /> RRF Fusion + Cross-Encoder Reranking</h3>
                {[
                  ...
                  <div key={s.step} className="flex gap-3 py-2.5 border-b border-slate-800 last:border-0">
                    <span className="shrink-0 w-6 h-6 rounded-full bg-indigo-950 text-indigo-400 text-xs font-black flex items-center justify-center">{s.step}</span>
              ...
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                <h3 className="font-bold text-slate-200 mb-4 flex items-center gap-2"><ShieldCheck className="text-emerald-600 w-5 h-5" /> Source Trust Hierarchy</h3>
                {[
                  { source: "WHO / NICE / NIH Guidelines", score: "0.95+", mult: "1.20x", color: "text-emerald-700" },
                  { source: "Cochrane Systematic Reviews", score: "0.90", mult: "1.15x + 0.12 boost", color: "text-emerald-700" },
                  { source: "Randomized Clinical Trials", score: "0.85", mult: "1.10x", color: "text-blue-700" },
                  { source: "NEJM / Lancet / JAMA", score: "0.85", mult: "+0.08 boost", color: "text-blue-700" },
                  { source: "Observational Studies", score: "0.75", mult: "0.95x", color: "text-amber-700" },
                  { source: "Expert Opinion / Case Reports", score: "0.65", mult: "0.85x", color: "text-orange-700" },
                  { source: "Unverified Internet Sources", score: "0.45", mult: "DROPPED (< 0.85 floor)", color: "text-red-700" },
                ].map((s, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-slate-800 last:border-0">
                    <span className="text-sm text-slate-350">{s.source}</span>
                    <div className="flex items-center gap-3 shrink-0">
                      <span className={`text-xs font-bold ${s.color}`}>{s.score}</span>
                      <span className="text-[10px] text-slate-400">{s.mult}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === "multimodal" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {[
                {
                  icon: Activity, color: "text-red-600", bg: "bg-red-50 border-red-200",
                ...
                {
                  icon: Microscope, color: "text-blue-600", bg: "bg-blue-50 border-blue-200",
                ...
                {
                  icon: FileText, color: "text-violet-600", bg: "bg-violet-50 border-violet-200",
                ...
                <div key={i} className={`p-5 rounded-2xl border ${p.bg} shadow-sm`}>
                  ...
                  <ol className="space-y-2">
                    {p.steps.map((s, j) => (
                      <li key={j} className="flex items-start gap-2 text-sm">
                        <span className="shrink-0 w-4 h-4 rounded-full bg-slate-900 text-slate-350 text-[9px] font-black flex items-center justify-center">{j+1}</span>
                        <span className="text-slate-350">{s}</span>
                      </li>
                    ))}
                  </ol>
                </div>
            ...
            <div className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
              <h3 className="font-bold text-slate-200 mb-3 flex items-center gap-2"><Layers className="text-indigo-400 w-5 h-5"/> Modality Router</h3>
              <p className="text-slate-400 text-sm leading-relaxed">
                When a file is uploaded, the <code className="text-indigo-400 bg-indigo-950 px-1.5 py-0.5 rounded font-mono text-xs">ImageIngestor</code> automatically classifies the modality using a vision model. ECG files are routed to the <code className="text-red-700 bg-red-50 px-1.5 py-0.5 rounded font-mono text-xs">EcgPipeline</code>, X-Rays to the <code className="text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded font-mono text-xs">RadiologyPipeline</code>, and text/PDFs to the <code className="text-violet-700 bg-violet-50 px-1.5 py-0.5 rounded font-mono text-xs">OcrPipeline</code>. All extracted data is fused into the patient context before the retrieval agents run — so the LLM always has both image-derived and text-derived evidence available simultaneously.
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === "governance" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                <h3 className="font-bold text-slate-200 mb-4 flex items-center gap-2"><Shield className="text-rose-600 w-5 h-5" /> Escalation Triggers</h3>
                {[
                  ...
                  <div key={e.trigger} className="flex items-start gap-3 py-2.5 border-b border-slate-800 last:border-0">
                    <code className="text-[9px] font-black text-amber-800 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded shrink-0">{e.trigger}</code>
              ...
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                <h3 className="font-bold text-slate-200 mb-4 flex items-center gap-2"><Users className="text-violet-600 w-5 h-5" /> Clinician Actions</h3>
                {[
                  ...
                  <div key={a.action} className="py-3 border-b border-slate-800 last:border-0">
                    <div className="font-semibold text-slate-200 text-sm">{a.action}</div>
                    <p className="text-sm text-slate-400 mt-0.5">{a.desc}</p>
                  </div>
                ))}
                <div className="mt-4 p-3 rounded-lg bg-indigo-950/40 border border-indigo-900/20">
                  <p className="text-xs text-indigo-400">All governance actions are written to an immutable audit log with actor identity, timestamp, and decision context — providing a full HITL audit trail.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {[
                ...
                <div key={i} className="p-5 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                  <h3 className="font-semibold text-slate-200 mb-3 flex items-center gap-2"><c.icon className="text-indigo-400 w-4 h-4"/>{c.title}</h3>
                  <ul className="space-y-1.5">
                    {c.items.map((it, j) => <li key={j} className="text-xs text-slate-400 flex items-start gap-1.5"><span className="text-slate-500 shrink-0">·</span>{it}</li>)}
                  </ul>
                </div>
              ))}
            </div>
        ...
        {activeTab === "usecases" && (
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {USE_CASES.map((u, i) => (
                <div key={i} className="p-5 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm flex flex-col gap-3">
                  <div className="flex items-start justify-between">
                    <span className="text-3xl">{u.emoji}</span>
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded border ${u.riskColor}`}>{u.risk} RISK</span>
                  </div>
                  <h3 className="font-bold text-slate-200">{u.title}</h3>
                  ...
                  <div className="pt-3 border-t border-slate-800 space-y-1">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {u.tags.map(t => <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-slate-900 text-slate-350 border border-slate-800">{t}</span>)}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {TECH_LAYERS.map((layer, i) => (
                <div key={i} className="p-6 rounded-2xl border border-slate-800 bg-slate-950 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {layer.items.map((it, j) => (
                      <div key={j} className="flex items-start gap-3">
                        <code className="text-xs font-bold text-indigo-400 bg-indigo-950 px-1.5 py-0.5 rounded shrink-0 min-w-[120px]">{it.name}</code>
...
const AGENTS = [
  ...
  { name: "Retrieval Agent", icon: Database, color: "text-emerald-600", tags: ["Async", "Parallel Execution", "Tool Calling"], desc: "Executes the Decision Plan by firing concurrent async tool calls to Qdrant (dense+sparse), Neo4j (Cypher traversal), and PubMed (live API). Gathers up to 40 raw candidate chunks into a shared context pool." },
  { name: "Evidence Evaluator", icon: BarChart3, color: "text-amber-600", tags: ["Critique", "Trust Scoring", "Quality"], desc: "Scores each retrieved chunk for relevance, recency, source authority, and trust. Computes a high_quality_count (quality >= 0.75) and an overall_sufficiency rating. If evidence is 'weak', it flags to the orchestrator." },
  { name: "Contradiction Analyzer", icon: Zap, color: "text-orange-600", tags: ["Cross-Source", "Conflict Detection"], desc: "Compares evidence chunks from different sources for clinical contradictions. Computes a severity penalty, identifies which sources conflict, and sets escalation_required=True if contradictions cannot be resolved automatically." },
  { name: "Reasoning Agent", icon: FileText, color: "text-violet-600", tags: ["Synthesis", "Llama-3-70b", "Structured Output"], desc: "The final LLM call. Takes the validated, filtered, high-quality evidence and generates a structured clinical report broken into sections (Diagnosis, Evidence, Recommendations, Caveats). Each claim is grounded to a source." },
  { name: "Validation Agent", icon: ShieldCheck, color: "text-green-600", tags: ["Grounding", "Safety", "Mandatory"], desc: "Evaluates the reasoning output for: evidence coverage, grounding score, source diversity, temporal coverage, and contradiction flags. Computes a composite validation_score. If it fails, triggers the Reflection Agent." },
  { name: "Reflection Agent", icon: RefreshCw, color: "text-rose-600", tags: ["Self-Healing", "HyDE", "Retry"], desc: "Activated on validation failure. Analyzes which sub-score failed (e.g. low source diversity), reformulates the query using HyDE (Hypothetical Document Embeddings), and forces a broader retrieval sweep before re-validation." },
...
const RAG_ENGINES = [
  {
    icon: Database, iconBg: "rgba(16,185,129,0.1)", iconColor: "#059669",
  ...
  {
    icon: Network, iconBg: "rgba(99,102,241,0.1)", iconColor: "#4f46e5",
  ...
  {
    icon: Globe, iconBg: "rgba(59,130,246,0.1)", iconColor: "#2563eb",
...
const USE_CASES = [
  { emoji: "🚨", title: "Emergency Triage", risk: "CRITICAL", riskColor: "bg-red-50 text-red-700 border-red-200", desc: "Patient presenting with ST elevations and acute chest pain. Aegis uses EMERGENCY workflow with CRITICAL strictness (0.95 threshold), HYBRID_STRICT retrieval, and single-retry policy for maximum speed with safety.", tags: ["EMERGENCY workflow", "1-retry limit", "Auto-escalation"] },
  { emoji: "💊", title: "Pharmacological Audit", risk: "HIGH", riskColor: "bg-orange-50 text-orange-700 border-orange-200", desc: "Checking contraindications for a patient on Warfarin prescribed a macrolide antibiotic. Neo4j graph traversal finds the drug-drug interaction chain. MEDICATION workflow with STRICT validation (0.82) is activated.", tags: ["MEDICATION workflow", "Neo4j traversal", "STRICT validation"] },
  { emoji: "🔍", title: "Rare Disease Matching", risk: "MEDIUM", riskColor: "bg-amber-50 text-amber-700 border-amber-200", desc: "Patient with undifferentiated auto-immune symptoms. SIMILAR_CASE workflow activates, embedding vitals and finding semantic matches in historical EHRs. Live PubMed search fetches latest orphan disease studies.", tags: ["SIMILAR_CASE workflow", "Case memory search", "PubMed live"] },
  { emoji: "📊", title: "Literature Synthesis", risk: "LOW", riskColor: "bg-blue-50 text-blue-700 border-blue-200", desc: "Physician querying the latest consensus on Long-COVID treatment. LITERATURE workflow fires INTERNET_AUGMENTED retrieval with AGGRESSIVE reflection (3 retries), synthesizing a meta-analysis from 15+ recent papers.", tags: ["LITERATURE workflow", "Internet augmented", "AGGRESSIVE reflection"] },
  { emoji: "🩻", title: "Radiology Review", risk: "HIGH", riskColor: "bg-orange-50 text-orange-700 border-orange-200", desc: "Chest X-Ray uploaded alongside clinical notes. RadiologyPipeline runs GPT-4o Vision to detect infiltrates, then Aegis cross-references findings with pneumonia guidelines from Qdrant and computes treatment plan.", tags: ["MULTIMODAL workflow", "GPT-4o Vision", "Guideline cross-reference"] },
  { emoji: "📝", title: "Discharge Summarization", risk: "LOW", riskColor: "bg-blue-50 text-blue-700 border-blue-200", desc: "Generating a patient-friendly discharge letter from structured clinical data. CLINICAL workflow with RELAXED validation (0.55) allows faster processing. OCR pipeline extracts data from uploaded lab PDFs automatically.", tags: ["CLINICAL workflow", "RELAXED validation", "OCR pipeline"] },
];
...
const TECH_LAYERS = [
  ...
  {
    icon: Database, color: "text-emerald-600", title: "Data & Storage Layer",
    items: [
  ...
  {
    icon: Workflow, color: "text-violet-600", title: "Orchestration Layer",
    items: [
  ...
  {
    icon: Globe, color: "text-blue-600", title: "Frontend & Deployment",
    items: [
  ...
  {
    icon: ShieldCheck, color: "text-rose-600", title: "Safety & Observability",
    items: [
  ...
  {
    icon: BookOpen, color: "text-amber-600", title: "Research & Ingestion",
    items: [
      ...
      { name: "PaddleOCR", role: "Primary OCR engine for text extraction" },
